...
export async function GET(request: NextRequest) {
  ...
  if (admins) {
    for (const admin of admins) {
      await supabase.from("notifications").insert({
        user_id: admin.id,
        title: `✅ Vermietung bestätigt: ${data.customer?.name}`,
  ...
  return new NextResponse(successPage(data.customer?.name, data.location?.name), { headers: { "Content-Type": "text/html" } });
}

function successPage(customer: string, location: string) {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Bestätigung</title></head>
  <body style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;margin:0;padding:40px 20px;background:#f5f5f7;min-height:100vh;display:flex;align-items:center;justify-content:center">
    <div style="max-width:440px;width:100%;text-align:center">
      <div style="background:white;padding:40px;border-radius:16px;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
        ...
        <h1 style="margin:0 0 8px;font-size:22px;color:#1a1a1a">Vermietung bestätigt!</h1>
        <p style="margin:0 0 20px;color:#666;font-size:15px">Vielen Dank${customer ? ", " + customer : ""}. Die Vermietung${location ? " für " + location : ""} wurde verbindlich bestätigt.</p>
        <p style="margin:0;color:#999;font-size:13px">Wir werden uns bezüglich der Übergabe bei Ihnen melden.</p>
      </div>
