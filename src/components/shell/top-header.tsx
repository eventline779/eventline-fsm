...
export function TopHeader() {
  ...
  return (
    <header className="hidden md:flex sticky top-0 z-20 h-14 items-center justify-center px-6 border-b border-border bg-background/80 backdrop-blur-md">
      <button
        type="button"
        onClick={openPalette}
        className="group flex items-center gap-2.5 pl-3.5 pr-2 h-9 rounded-full border border-border/80 bg-foreground/[0.02] dark:bg-white/[0.04] dark:border-white/10 text-muted-foreground hover:text-foreground hover:border-border hover:bg-foreground/[0.05] dark:hover:bg-white/[0.07] dark:hover:border-white/20 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-500/40 transition-all w-full max-w-lg text-[13px]"
      ...
      >
        ...
        <kbd className="inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-md border border-border/60 dark:border-white/10 bg-background/60 dark:bg-white/[0.05] text-muted-foreground/80 shrink-0 tracking-wide">
          {mac ? "⌘K" : "Ctrl K"}
        </kbd>
      </button>
    </header>
