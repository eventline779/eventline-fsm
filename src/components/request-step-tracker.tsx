...
// Strukturell: outer flex ist items-start. Kreise stehen oben, Labels (im lg-Modus)
// drunter. Die Verbindungslinien zwischen Kreisen werden per margin-top auf die
// Mitte des Kreises gesetzt (mt = halbe Kreis-Hoehe minus halbe Linien-Hoehe), damit
// sie unabhaengig von Label-Wraps immer auf Kreis-Mittellinie bleiben.
//
//   ⬤━━⬤━━⬤━━⬤━━⬤   ← alle Kreise auf gleicher Hoehe, Linien in Flucht der Mittellinien
//   |   |   |   |   |
//   Lbl Lbl Lbl Lbl Lbl  ← Labels wrap-frei, manche eine, manche zwei Zeilen
const DOT_PX_LG = 28; // w-7 / h-7
const DOT_PX_SM = 20; // w-5 / h-5
const LINE_PX = 2;

export function RequestStepTracker({ currentStep, size = "lg", className = "" }: RequestStepTrackerProps) {
  const isLg = size === "lg";
  const dotPx = isLg ? DOT_PX_LG : DOT_PX_SM;
  const lineMarginTop = (dotPx - LINE_PX) / 2; // damit Linie auf Kreis-Mittellinie liegt
  ...
  return (
    <div className={`flex items-start ${isLg ? "gap-2" : "gap-1"} ${className}`}>
      {REQUEST_STEPS.map((s, i) => {
        ...
        return (
          <Fragment key={s.step}>
            <div className={`flex flex-col items-center ${isLg ? "gap-1.5" : "gap-0.5"}`}>
              <div className={`${dotSize} shrink-0 rounded-full flex items-center justify-center font-semibold border transition-colors ${stateClasses}`}>
            ...
            {i < REQUEST_STEPS.length - 1 && (
              <div
                style={{ marginTop: `${lineMarginTop}px` }}
                className={`${lineWidth} h-[2px] shrink-0 rounded-full transition-colors ${done ? "bg-[var(--status-green)]" : "bg-foreground/10"}`}
              />
            )}
          </Fragment>
        );
